...
export function Sidebar({
    ...
    const hasActiveFilters = searchTerm || activeCategoryIds.length > 0;
    ...
    return (
        <aside className="absolute top-6 left-6 bottom-6 w-80 bg-background/80 backdrop-blur-sm rounded-2xl shadow-lg flex flex-col gap-4 z-10 p-4">
            ...
            <div className="flex items-center justify-between px-2 h-8">
                ...
                {hasActiveFilters && (
                    <Button variant="ghost" size="sm" onClick={onClearFilters} className="text-muted-foreground h-auto p-1">Clear<X className="ml-1 h-4 w-4" /></Button>
                )}
            </div>
