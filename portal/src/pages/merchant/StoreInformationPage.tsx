...
function EditDescriptionDialog({ children, place, onUpdate, isPending }: DialogProps<DescriptionFormValues>) {
    ...
    return <Dialog open={isOpen} onOpenChange={setIsOpen}><DialogTrigger asChild>{children}</DialogTrigger><DialogContent><DialogHeader><DialogTitle>Edit Description</DialogTitle><DialogDescription>Provide a detailed description for your store.</DialogDescription></DialogHeader><form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4" autoComplete="off"><Textarea {...register("description")} rows={8} /><p className="text-sm text-red-500">{errors.description?.message}</p><DialogFooter><Button type="submit" disabled={isPending}>{isPending ? 'Saving...' : 'Save Changes'}</Button></DialogFooter></form></DialogContent></Dialog>
}
...
function EditImageDialog({ children, field, title, currentImageUrl, onUpdate, isPending }: ImageDialogProps) {
    ...
    return <Dialog open={isOpen} onOpenChange={onOpenChange}><DialogTrigger asChild>{children}</DialogTrigger><DialogContent><DialogHeader><DialogTitle>{title}</DialogTitle><DialogDescription>{field === 'cover' ? "Upload a new banner. 16:9 ratio recommended." : "Upload a new logo. 1:1 (square) ratio recommended."}</DialogDescription></DialogHeader><form onSubmit={handleSubmit(onSubmit)} className="space-y-4 py-4" autoComplete="off"><div className="flex justify-center bg-muted/50 rounded-lg p-4">{preview ? <img src={preview} alt="Preview" className={`object-contain rounded-md border max-h-60 ${field === 'logo' ? 'aspect-square' : 'aspect-video'}`} /> : <div className={`flex items-center justify-center bg-muted rounded-md h-48 ${field === 'logo' ? 'w-48' : 'w-full'}`}><ImageIcon className="h-12 w-12 text-muted-foreground" /></div>}</div><div className="space-y-2"><Label htmlFor={field}>New Image</Label><Input id={field} type="file" accept="image/jpeg,image/png,image/gif,image/svg+xml" {...register(field, { onChange: handleFileChange })} /></div><DialogFooter><Button type="submit" disabled={isPending}>{isPending ? 'Uploading...' : 'Upload & Save'}</Button></DialogFooter></form></DialogContent></Dialog>;
}
